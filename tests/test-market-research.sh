...
if [ -f "$SESSION_DIR/output/mission-report.md" ]; then
    ...
    if grep -q "TAM\|SAM\|SOM\|Total Addressable Market" "$SESSION_DIR/output/mission-report.md"; then
    ...
    if grep -q "competitors\|Competitive\|market share" "$SESSION_DIR/output/mission-report.md"; then
    ...
    if grep -q "funding\|revenue\|valuation\|ARR\|growth" "$SESSION_DIR/output/mission-report.md"; then
        echo "✓ Financial metrics included"
    fi

    # Check for market sources
    if grep -q "Crunchbase\|market report\|Gartner\|Forrester" "$SESSION_DIR/output/mission-report.md"; then
        echo "✓ Market research sources cited"
    fi

    # Check for data quality notes
    if grep -q "disclosed\|estimated\|methodology" "$SESSION_DIR/output/mission-report.md"; then
