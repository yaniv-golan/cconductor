...
if [ -f "$SESSION_DIR/70_report/mission-report.md" ]; then
    ...
    if grep -q "arXiv\|Google Scholar\|pubmed\|ieee" "$SESSION_DIR/70_report/mission-report.md"; then
    ...
    if grep -q "Methodology\|methodology\|experimental\|statistical" "$SESSION_DIR/70_report/mission-report.md"; then
    ...
    if grep -q "peer.review\|journal\|conference" "$SESSION_DIR/70_report/mission-report.md"; then
        echo "✓ Peer review status discussed"
    fi

    # Check for citation network
    if grep -q "Citation\|citation\|References" "$SESSION_DIR/70_report/mission-report.md"; then
