...
- Paragraph-level evidence pipeline is now documented in `USAGE.md`, including guidance for footnote rendering and the CommonMark-compatible fallback workflow.

### Changed

- `cconductor` now defaults to `--evidence-mode render` (with the `footnotes` renderer) so paragraph-level citations are enabled out of the box.
- Removed hard-coded version strings from user-facing documentation to avoid stale references between releases.
- Mission reports are post-processed with paragraph evidence, replacing the manual “Sources and Citations” section with autogenerated footnotes (or fallback evidence blocks).
