...
score_claim_confidence() {
    ...
    local sources=$(echo "$claim_json" | jq '.sources | length')
    local evidence_quality=$(echo "$claim_json" | jq -r '.evidence_quality // "medium"')
    ...
    local confidence=$(safe_calculate "$source_score * $quality_mult" | jq -r '.result' | awk '{printf "%.2f", $0}')
...
calculate_overall_confidence() {
    ...
    local claims=$(echo "$kg_json" | jq '.claims')
    local claim_count=$(echo "$claims" | jq 'length')
    ...
    local total_confidence=$(echo "$claims" | jq '[.[].confidence // 0] | add')
    local avg_confidence=$(safe_calculate "$total_confidence / $claim_count" | jq -r '.result' | awk '{printf "%.2f", $0}')

    # Penalize if many gaps remain
    local gaps=$(echo "$kg_json" | jq '.stats.unresolved_gaps // 0')
    local gap_penalty=0
    ...
    local contradictions=$(echo "$kg_json" | jq '.stats.unresolved_contradictions // 0')
    ...
    local final_confidence=$(safe_calculate "$avg_confidence - $gap_penalty - $contra_penalty" | jq -r '.result' | awk '{printf "%.2f", $0}')
    ...
    local categories=$(echo "$kg_json" | jq '[.entities[].type] | unique')
    local by_category='{}'

    while IFS= read -r category; do
        if [ "$category" != "null" ] && [ -n "$category" ]; then
            # Get claims related to entities of this category
            local category_entities=$(echo "$kg_json" | jq -c --arg cat "$category" \
                '[.entities[] | select(.type == $cat) | .name]')

            local category_claims=$(echo "$kg_json" | jq --argjson entities "$category_entities" \
                '[.claims[] | select(.related_entities as $re | $entities | any(. as $e | $re | contains([$e])))]')

            local cat_count=$(echo "$category_claims" | jq 'length')

            if [ "$cat_count" -gt 0 ]; then
                local cat_confidence=$(echo "$category_claims" | jq '[.[].confidence // 0] | add / length' | awk '{printf "%.2f", $0}')
...
calculate_coverage() {
    ...
    local partial_covered=$(echo "$kg_json" | jq \
    ...
    local not_covered=$((entities - well_covered - partial_covered))
