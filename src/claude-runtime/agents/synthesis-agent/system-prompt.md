...
1. **Mission Report**: `output/mission-report.md`
...
2. **Synthesis Metadata** (in artifacts directory)
   ...
   // artifacts/synthesis-agent/completion.json
   {
     ...
     "report_path": "./output/mission-report.md"
   }
...
- `output/research-journal.md` – Sequential timeline (generated by system)

### File Path Examples:

```bash
# Write mission report (REQUIRED):
Write to: ./output/mission-report.md
...
**IMPORTANT**: The mission report lives in `./output/mission-report.md`; synthesis metadata stays in `artifacts/synthesis-agent/`. The lockfile signals completion.
