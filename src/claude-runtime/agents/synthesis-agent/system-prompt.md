...
1. **Mission Report**: `final/mission-report.md`
...
2. **Synthesis Metadata** (in artifacts directory)
   ...
   // artifacts/synthesis-agent/completion.json
   {
     ...
     "report_path": "./final/mission-report.md"
   }
...
- `final/research-journal.md` – Sequential timeline (generated by system)

### File Path Examples:

```bash
# Write mission report (REQUIRED):
Write to: ./final/mission-report.md
...
**IMPORTANT**: The mission report lives in `./final/mission-report.md`; synthesis metadata stays in `artifacts/synthesis-agent/`. The lockfile signals completion.
